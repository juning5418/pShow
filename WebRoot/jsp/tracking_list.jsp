...
<body>
	<div class="pageHeader">
		<form rel="pagerForm" onsubmit="return navTabSearch(this);"
			action="tracking_list" method="post">
	...
	<form rel="pagerForm" onsubmit="return navTabSearch(this);"
		...
		<div class="pageContent">
			<div class="panelBar">
				<ul class="toolBar">
					<li><a class="add" href="add_tracking?projectId=${projectId}" target="navTab"><span>添加</span>
					</a></li>
			...
			<table class="table" width="98%" layoutH="138" asc="asc" desc="desc">
				<thead>
					<tr>
						...
						<th width="" orderField="title" class="${orderDirection}">标题</th>
						<th width="" orderField="content" class="${orderDirection}">内容</th>
						<th width="" orderField="addTime" class="${orderDirection}">增加时间</th>
						<th width="" orderField="editTime" class="${orderDirection}">修改时间</th>
						<th width="">操作</th>
					</tr>
				</thead>

				<tbody id="tbody">
					<c:forEach items="${trackingList}" var="track">
						...
						<!-- 	<td><input name="ids" value="${pro.id}" type="checkbox">
							...
							<td>${track.content}</td>
							<td>${track.addTime}</td>
							<td>${track.editTime }</td>
							<td><a title="删除" target="ajaxTodo"
								href="del_tracking?id=${track.id}" class="btnDel"
								callback="navTabAjaxDone">删除</a> <a title="编辑" target="navTab"
								href="edit_tracking?id=${track.id}" class="btnEdit">编辑</a>
							</td>
						</tr>
					</c:forEach>
				</tbody>
